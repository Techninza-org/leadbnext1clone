    ...
    >
      {value.map((item, index) => (
        <Badge
          key={item}
          className={cn(
            "px-1 rounded-xl flex text-xs p-1 items-center gap-1",
...
>(({ className, children }, ref) => {
  return (
    <CommandList
      ref={ref}
      className={cn(
        "p-2 flex flex-col gap-3 rounded-md scrollbar-thin scrollbar-track-transparent transition-colors scrollbar-thumb-muted-foreground dark:scrollbar-thumb-muted scrollbar-thumb-rounded-lg w-full absolute bg-background shadow-md z-10 border border-muted top-0",
        className
      )}
    >
      {children}
...
>(({ className, value, children, ...props }, ref) => {
  ...
  return (
    <CommandItem
      ...
      className={cn(
        "rounded-md cursor-pointer p-2 transition-colors flex justify-between ",
        className,
